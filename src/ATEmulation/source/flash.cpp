...
void ATFlashEmulator::Init(void *mem, ATFlashType type, ATScheduler *sch) {
	...
	switch(mFlashType) {
		...
		case kATFlashType_MX29F001T:
		case kATFlashType_MX29F002T:
		case kATFlashType_MX29F002B:
			...
			if (mFlashType == kATFlashType_S29GL01P ||
				...
				case 0x00:
					switch(mFlashType) {
						...
						case kATFlashType_SST39SF010:
						case kATFlashType_SST39SF020:
		...
		case 5:		// 5555[AA] 2AAA[55] 5555[80] 5555[AA] 2AAA[55]
			...
			if (value == 0x10 && (mbA12iUnlock ? (address & 0xFFF) == 0xAAA : mbA11Unlock ? address11 == 0x555 : address15 == 0x5555)) {
				...
				switch(mFlashType) {
					...
					case kATFlashType_SST39SF010:
					case kATFlashType_MX29F001T:
						memset(mpMemory, 0xFF, 0x20000);		// 128K (1Mbit)
						break;
					...
					case kATFlashType_MX29F002T:
					case kATFlashType_SST39SF020:
						memset(mpMemory, 0xFF, 0x40000);		// 256K (2Mbit)
						break;
					...
					case kATFlashType_M29W800DT:
					case kATFlashType_MX29LV640DT:
						memset(mpMemory, 0xFF, 0x800000);		// 8M (64Mbit)
						break;

					case kATFlashType_Am29F016D:
						memset(mpMemory, 0xFF, 0x200000);		// 2M (16Mbit)
						break;
...
void ATFlashEmulator::SectorErase(uint32 address) {
	switch(mFlashType) {
		case kATFlashType_Am29F010:
		case kATFlashType_Am29F010B:
		case kATFlashType_M29F010B:
		...
		case kATFlashType_M29W800DT:
			{
				...
				memset(mpMemory + address, 0xFF, blockSize);
				g_ATLCFlash("Erasing block $%07X-%07X\n", address, address + blockSize - 1);
			}
			break;
